...
export default function ControlPannel({ saveBlogOnClicked ,blogCategory , blogName, doc}: Props) {
  ...
  const checkMousePosition = (e: MouseEvent) => {
    ...
    if (nearBottom && nearLeft) {
      ...
      setTimeout(() => setShowControlPanel(false), 10000)
    }
  }
  ...
  return (
    <div className={`px-4 py-3 flex flex-row justify-around gap-4 items-center m-0 controlPannel fixed ${showControlPanel ? 'bottom-16 ' : '-bottom-16'} left-12 transition-all rounded bg-gray-600 bg-opacity-50 backdrop-blur-md`}>
